...
// POST /api/projects?action=migrate-works — 迁移旧作品到最近的项目
export async function PATCH(request: NextRequest) {
  try {
    ...
    if (!orphanWorks || orphanWorks.length === 0) {
      return NextResponse.json({ migrated: 0, message: '没有需要迁移的作品' });
    }

    // 获取最近的项目
    const { data: recentProject, error: projectError } = await supabase
      .from('projects')
      .select('id, name')
      .eq('user_id', authResult.user.id)
      .order('created_at', { ascending: false })
    ...
    if (recentProject) {
      ...
      const { data: newProject, error: createError } = await supabase
        .from('projects')
        .insert({ user_id: authResult.user.id, name: dateName, is_default: true })
        .select()
        .single();
    ...
    return NextResponse.json({ migrated: orphanWorks.length, project_id: projectId });
  } catch (err) {
    console.error('Works migration error:', err);
    return NextResponse.json({ error: '迁移作品失败' }, { status: 500 });
  }
}
